...
export class ActivityModel {
  ...
  static async getAllByCodeCase(codeCase) {
    try {
      ...
      console.error(
        `Error retrieving activities by code case (${codeCase}): ${error.message}`
      );
      // Log the full error for more details if needed
      console.error(error);
      throw new Error(
        `Error retrieving activities by code case: ${error.message}`
      );
    }
  }
  ...
  static async getAllById(internalId) {
    try {
      ...
      throw new Error(
        `Error retrieving activities by Internal_ID: ${error.message}`
      );
    }
  }
  ...
  static async update(id, activityData, file = null, internalUser) {
    ...
    try {
      ...
      for (const key in activityData) {
        if (
          activityData.hasOwnProperty(key) &&
          activityData[key] !== originalValues[key]
        ) {
          changeDetails.push(
  ...
  static async generateExcelReportForActivities(startDate, endDate) {
    try {
      ...
      worksheet.columns = [
        ...
        {
          header: "Apellidos y Nombres Usuario/a",
          key: "user_name",
          width: 30,
        },
        ...
        {
          header: "Nro. Juzgado / Unidad Judicial",
          key: "court_number",
          width: 30,
        },
        {
          header: "Última Actividad o Diligencia",
          key: "last_activity",
          width: 40,
        },
        {
      ...
      activities.forEach((activity, index) => {
        ...
        const row = worksheet.addRow({
          ...
          subject_topic: `${consultation.Init_Subject || ""} >> ${
            consultation.Init_Topic || ""
          }`,
          user_name: `${user.User_FirstName || ""} ${user.User_LastName || ""}`,
          user_id: user.User_ID || "N/A",
          birth_date: user.User_BirthDate
            ? moment(user.User_BirthDate).format("DD/MM/YYYY")
            : "N/A",
